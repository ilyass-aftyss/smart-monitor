...
type FluidTabSlotContextValue = { index: number };
...
function useFluidTabSlot() {
  ...
  if (!context) throw new Error("FluidTabs.Tab must be a direct child of <FluidTabs.List>.");
  return context;
}
...
function FluidTabsRoot({
  ...
  return (
    <FluidTabsContext.Provider
      value={{ activeIndex, setActiveIndex, focusedIndex, setFocusedIndex, indicatorLayoutId }}
    >
...
type FluidTabsListProps = ComponentProps<"nav"> & { "aria-label"?: string };
...
type FluidTabsTabProps = ComponentProps<"button"> & { label?: string };
  ...
  return (
    <button
      ...
      onFocus={(event: FocusEvent<HTMLButtonElement>) => {
        onFocus?.(event);
        if (!event.defaultPrevented) setFocusedIndex(index);
      }}
      className={cn(
