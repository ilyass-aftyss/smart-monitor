...
function NavItemRow({
  item, collapsed, active, alertCount, onNavigate,
}: {
...
export default function Sidebar({ collapsed, mobileOpen, sidebarWidth, onToggleCollapse, onCloseMobile, onResize }: SidebarProps) {
  ...
  const sidebarContent = (
    <Box sx={{
        ...
        <ClickAwayListener onClickAway={() => setUserOpen(false)}>
          <Box>
            <Box
              ...
              sx={{ display: 'flex', alignItems: 'center', gap: 1.5, justifyContent: collapsed ? 'center' : 'flex-start' }}
            >
              <Avatar sx={{
              ...
              {!collapsed && (
                <>
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography sx={{ fontSize: '0.78rem', fontWeight: 600, color: textPri, lineHeight: 1.2 }}>
                      {username}
                    </Typography>
            ...
            {!collapsed && (
              <Collapse in={userOpen}>
                <Box sx={{ pt: 1.5, borderTop: border, mt: 1.5 }}>
                  {[
                    { icon: User, label: 'Mon Compte', onClick: () => {} },
                    { icon: Settings, label: 'Parametres', onClick: () => handleNavigate('/admin') },
                    { icon: LockKeyhole, label: 'Verrouiller', onClick: () => {} },
                    { icon: LogOut, label: 'Deconnexion', onClick: () => { logout(); handleNavigate('/login') } },
                  ].map((item) => (
                    <Box
                      key={item.label}
                      ...
                      sx={{
                        ...
                        px: 1.5, py: 1.2, borderRadius: 1.5, cursor: 'pointer',
                        color: item.label === 'Deconnexion' ? '#EF4444' : '#6B7280',
                        fontSize: '0.78rem',
                        transition: 'all 0.15s',
                        '&:hover': {
      ...
      <Box sx={{ borderTop: border, p: collapsed ? 1 : 1.5, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
        <Box
          onClick={toggle}
          sx={{
